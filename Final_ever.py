...
def extract_tsfel_features(X, ev_id, dur = 1, domain = 'statistical'):
        ...
        X_features = pd.DataFrame(np.reshape(X_train, [len(X), len(feature_labels)]))
        
        X_features.columns = feature_labels
        X_features['EventID'] = ev_id
...
for i in tqdm(range(len(stations))):
            ...
            X,y, ev_id = load_raw_data(stations[i], dur =0.5, freq_band = [0.5,25])
            X_f = extract_tsfel_features(X, ev_id, dur = 0.5, domain = 'all')
            X_f.to_csv("/home/ak287/Icequakes_vs_Tectonicquakes/tsfel_raw_data_features/all/30s/"+stations[i]+".csv")
            #f_labels.to_csv("/home/ak287/Icequakes_vs_Tectonicquakes/tsfel_raw_data_features/all/30s/"+stations[i]+"_labels.txt", y)
