...
for station in stations:
    ...
    for i in tqdm(range(len(X))):
            ...
            df = time_series_features_extractor(cfg_file, X[i], fs= 50,)
            df['serial_no'] = total_index[i]
            df['label'] = y[i]
            df_features = pd.concat([df_features, df])

    df_features.to_csv('/home/ak287/Icequakes_vs_Tectonicquakes/Extracted_Features/Tsfel_features_1_25_'+station+'.csv')